...
Shader::Shader(QOpenGLExtraFunctions *functions, const std::string &filename)
...
void Shader::SetUniformMat4f(const std::string &name,
                             const s21::Matrix4x4 &mat) {
  m_functions->glUniformMatrix4fv(GetUniformLocation(name), 1, GL_FALSE,
                                  mat.GetData());
}
