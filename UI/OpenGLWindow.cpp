...
OpenGLWindow::OpenGLWindow(QWidget *parent) : QOpenGLWidget(parent) {
  ...
  lastFrame = std::chrono::high_resolution_clock::now(); // Инициализация времени
...
void OpenGLWindow::paintGL() {
  ...
  deltaTime = std::chrono::duration<float>(currentFrameTime - lastFrame).count(); // Вычисляем deltaTime
  lastFrame = currentFrameTime;
 
  std::cout << "deltatime " << deltaTime << std::endl;

  s21::Matrix4x4 projection;
  s21::Matrix4x4 view ;
